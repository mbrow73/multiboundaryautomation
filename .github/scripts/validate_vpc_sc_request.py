...
Outputs are written to $GITHUB_OUTPUT:
  ...
  - debug_sources: JSON array of source lists for each rule (for debugging)
"""
...
def parse_issue_body(issue_text: str) -> Dict[str, Any]:
    ...
    for match in rule_pattern.finditer(clean_text):
        ...
        def extract_values(label: str) -> List[str]:
            # Capture lines indented under the label
            pattern = rf"{label}[^\n]*\n((?:\s+[^\n]+\n)*)"
...
def main():
    ...
    # Collect parsed sources for each rule for debugging
    debug_sources = [rule.get("sources", []) for rule in parsed.get("rules", [])]
    ...
    if out_path:
        with open(out_path, "a") as out:
            ...
            out.write(f"debug_sources={json.dumps(debug_sources)}\n")
    ...
    # Print the parsed 'sources' lists so you can see what was captured
    print("Parsed 'sources' lists for each rule:", json.dumps(debug_sources))
